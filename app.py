...
# 🐛 MIDLERTIDIG DEBUG-KNAP
with st.sidebar:
    st.markdown("---")
    st.markdown("### 🐛 Debug")
    if st.button("🧪 Test yfinance NVDA"):
        ...
        # Test 1
        st.markdown("**Test 1: get_earnings_dates(limit=8)**")
        try:
            ed = tk.get_earnings_dates(limit=8)
            if ed is None:
                st.error("❌ Returnerede None")
            elif ed.empty:
                st.error("❌ Returnerede tom DataFrame")
            else:
                st.success(f"✅ {len(ed)} rows")
                st.code(f"Columns: {list(ed.columns)}")
                st.dataframe(ed.head())
        except Exception as e:
            st.error(f"❌ FEJL: {type(e).__name__}: {e}")
        
        # Test 2
        st.markdown("**Test 2: earnings_dates property**")
        try:
        ...
        # Test 3
        st.markdown("**Test 3: info dict**")
        try:
            info = tk.info
            st.success(f"✅ Info keys: {len(info)} keys")
            for key in ["earningsTimestamp", "earningsDate", "trailingEps"]:
                st.code(f"{key}: {info.get(key)}")
        except Exception as e:
            st.error(f"❌ FEJL: {e}")
